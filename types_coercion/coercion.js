////////// Abstract Operations //////////////
//These operators aren't accessible for use but they are important because they are run behind-the-scenes for all type conversions. They inlude ToPrimitive, ToString, ToNumber, ToBoolean. A lot of unexpected results originate in how these abstract operators work. The big one to memorize is that ToNumber("") returns 0.
...
baz; // 123, because parseInt loops through a string looking for integers. It stops when it encounters a non integer.
...
var baz = !!foo;
baz; //true
//The above is a very common way to create booleans. It works fine but why not use the Boolean native function? It is surely clearer that way and it works more directly. Using a double bang both coerces and negates, whereas using the Boolean native function just coerces. Do this instead:
var foo = "123";
var baz = Boolean(foo);
baz; // true
...
// The most common use of the Date constructor is to create a numeric timestamp. This can be easily done 2 ways.
var now = new Date();
now = now.getTime();
now; //1527195878550
typeof now; //number
...
baz; // 123
typeof baz; //number
// There are many mathematical operators that also work but keep in mind that if either operand is a string, the + operator always converts the operand to a string. So, use the + for implicitly converting from number to string, like shown below.
...
//Conditionals are always implicitly coerced to Booleans. Remember that any value not on the falsy list is coerced to true.

// Double equals
// It is commonly said that double equals evaluates value while triple equals evaluates value and type. This statement is simply false and proof comes straight from the spec.
// The correct statement that while they both test equality, double equals allows type coercion and triple equals doesn't. //Double equals follows understandable rules
...
null == undefined; // true but not because both are coverted to the number 0. It is because null and undefined are special cases, they are coercively equal to each other and no other value.
...
"0" == ""; //false but I'm not sure why - I would think both would be converted to 0 - The problem is that if both values are the same type, no conversion happens
