...
//It is common to say that callbacks cause readability problems because of nesting. There are 2 much deeper problems: Inversion of control, and the inability to nicely handle temporal dependency. Inversion of control simply means that you lose the ability to control when or how your callback is fired. What if you use a library that mistakenly calls your callback twice? It is possible to write a bunch of conditionals to ensure the callback fires correctly but that would require a lot of code. The temporal dependency issue can be shown with a simple example. Say you want to make 3 network requests and you want them to all render ASAP but they must render in order (first, second, third). Each network request will have to share state through closure and many conditionals will be necessary to ensure the responses get rendered both ASAP and in order (imagine trying to reason through all the conditionals if the temporal depency was more complex! It could get out of hand very easily). Async examples are usually simplified such that that new requests are made sequentially - that request 2 doesn't begin until request 1 has a response. That doesn't account for the complexity that comes from scenarios with temporal dependency. The callback pattern simply doesn't handle temporal dependency or "time state complexity" well.
